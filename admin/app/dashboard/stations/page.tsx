...
export default function StationsPage() {
  ...
  const [deleteModal, setDeleteModal] = useState<{
    ...
    stationId: string;
    stationName: string;
  }>({ show: false, stationId: "", stationName: "" });
  ...
  const loadStations = async () => {
    try {
      const data = await api.getAllStations();
      setStations(data);
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage all EV charging stations</p>
        </div>
        <Link
                  ...
                  <tr key={station._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-900">
                        {occupiedPorts}/{totalPorts} occupied
                      </span>
                    </td>

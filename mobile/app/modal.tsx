...
export default function NavigationScreen() {
  ...
  // Handle Android back button - also cleanup when hardware back is pressed
  useEffect(() => {
    const backHandler = () => {
      cleanup();
    ...
    // Note: This is handled by Expo Router automatically, but we ensure cleanup
    return () => {
      // Cleanup is already called in the main useEffect cleanup
    };
  }, [stationId]);
  ...
  const initializeNavigation = async () => {
    try {
      ...
      // Socket is already connected via SocketProvider
  ...
  const cleanup = () => {
    ...
    // Unsubscribe from station room (but keep socket connected)
  ...
  const handleClose = () => {
    ...
    if (router.canGoBack()) {
      ...
      // Fallback: navigate to stations tab
      router.replace("/stations");
    }
  };

...
const MetaBadge = styled.View<{ variant?: "primary" | "secondary" | "success" }>`
...
const OpenClosedBadge = styled.View<{ isOpen: boolean }>`
  flex-direction: row;
  align-items: center;
  background-color: ${({ isOpen }) => (isOpen ? "rgba(46, 125, 50, 0.1)" : "rgba(198, 40, 40, 0.1)")};
  padding: ${spacing.xs}px ${spacing.sm}px;
  border-radius: 4px;
`;
...
function NearbyStationItem({
  ...
  const stationOpen = station.operatingHours ? isStationOpen(station.operatingHours) : true;
  
  return (
    <StationCard onPress={onPress}>
      <StationThumbnail>
        {station.images && station.images.length > 0 ? (
          <StationThumbnailImage source={{ uri: station.images[0] }} resizeMode="cover" />
        ) : (
      ...
      <StationInfo>
        ...
        <StationMeta>
          <MetaBadge variant="primary">
            <MetaText variant="primary">{station.distanceKm.toFixed(1)} km</MetaText>
          </MetaBadge>
          <MetaBadge>
            <MetaText>
              {station.portSummary.carPorts} car • {station.portSummary.bikePorts} bike
            </MetaText>
          </MetaBadge>
      ...
      <StationInfo>
        ...
        <StationMeta>
          ...
          <MetaBadge variant="success">
            <MetaText variant="success">{station.freeSlots}/{station.totalSlots}</MetaText>
          </MetaBadge>
          <OpenClosedBadge isOpen={stationOpen}>
            <Ionicons 
              name={stationOpen ? "checkmark-circle" : "close-circle"} 
...
export default function StationsListScreen() {
  ...
  const { location, hasPermission, isLoading, requestPermission } = useLocation();
  const { hasResults, recommendations, radius, setNearbyStations, nearbyStations: contextNearbyStations } = useRecommendation();
